...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument('--users', '-r', type=int)
    ...
    def fill_profiles(self, amount):
        for i in range(amount):
            ...
            user.save()
            Profile.objects.create(rating=faker.random_int(min=-50, max=1000), avatar='/static/img/ava1.png', user=user)
    ...
    def fill_ratings(self, amount):
        ...
        profiles_id = list(Profile.objects.values_list('id', flat=True))
        for i in range(amount):
            rating_object = choice(
                [Question.objects.get(pk=choice(questions_id)), Answer.objects.get(pk=choice(answers_id))])
    ...
    def handle(self, *args, **options):
        ...
        self.fill_profiles(users)
        self.fill_tags(users)
        self.fill_questions(10 * users)
        self.fill_answers(100 * users)
        self.fill_ratings(200 * users)
